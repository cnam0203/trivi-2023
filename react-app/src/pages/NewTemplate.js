...
export default () => {  
  ...
  TabTitle(`New ${itemType} template`);
  ...
  const handleFormatData = (results) => {
    ...
    if (data.length > 0) {
      const columns = data[0].map(key => {return key}).sort();
      setColumns([...columns]);
    }
  }
  ...
  const handleImportFile = (e) => {
    ...
    fetchRequest(`data/import-matching-template/${itemType}`, 'POST', 
    JSON.stringify({
      ...
      <Container className="px-0">
        <Row className="d-flex flex-wrap flex-md-nowrap align-items-center pt-3">
          <Col className="d-block mb-2 mb-md-0">
            <h1 className="h2">New {itemType} template</h1>
          </Col>
        </Row>
        <Row className="d-flex flex-wrap flex-md-nowrap align-items-center py-3">
          ...
          <Form className="row" onSubmit={(e) => handleOpenModal(e)}>
                ...
                <Form.Group className="mb-3 col-6">
                    <Form.Label>Upload a sample file</Form.Label>
                    <Form.Control 
                        type="text"
