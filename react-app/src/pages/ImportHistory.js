...
export default () => {
  ...
  useEffect(() => {
    fetchRequest(`data/get-import-history/${itemType}`, 'GET')
    .then((data) => {
      if (data != undefined) {
  ...
  const handleDeleteItem = () => {
    fetchRequest(`data/delete-import-log/${deleteItem}`, 'DELETE')
    .then((data) => {
      if (data != undefined) {
        if (data.status == 200) {
  ...
  const handleViewDetail = (row) => {
    const id = row["id"];
    const url = `/data-management/list/${itemType}?import_id=${id}`;
    history.push(url);
  };
      ...
      <Container className="px-0">
        <React.Fragment>
          ...
          >
            ...
            <Modal.Body>
              <p>Do you want to delete all logs in this import time?</p>
            </Modal.Body>
            <Modal.Footer>
              <Button
                  variant="secondary"
        ...
        <Row className="d-flex flex-wrap flex-md-nowrap align-items-center py-4">
          <Col className="d-block mb-4 mb-md-0">
            <h1 className="h2">Import history - {itemType}</h1>
          </Col>
        </Row>
        <Row>
