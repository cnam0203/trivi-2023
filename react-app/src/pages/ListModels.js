...
export default () => {
  ...
  useEffect(() => {
    ...
    fetchRequest(`knowledge/get-list-models/${itemType}${importId ? `?import_id=${importId}` : ''}`, 'GET')
    .then((data) => {
      if (data != undefined) {
  ...
  const handleDeleteItem = () => {
    fetchRequest(`knowledge/delete-model/${itemType}/${deleteItem}`, 'DELETE')
    .then((data) => {
      if (data != undefined) {
        if (data.status == 200) {
  ...
  const handleViewDetail = (row) => {
    const id = row["id"];
    const url = `/data-knowledge/detail/${itemType}/${id}`;
    history.push(url);
  };

  const handleNewModel = (e) => {
    const url = `/data-knowledge/run/${itemType}`;
    history.push(url);
  }
