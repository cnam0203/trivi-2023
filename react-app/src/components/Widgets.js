...
export const CounterWidget = ({info}) => {
  const percentageIcon = info['percentage'] < 0 ? faAngleDown : faAngleUp;
  const percentageColor = info['percentage'] < 0 ? "text-danger" : "text-success";
        ...
        <Row className="d-block d-xl-flex">
          <Col xs={12} xl={12} className="px-xl-0">
            <div className="d-none d-sm-block">
              <h3 className="mb-1 text-center">{info['title']}</h3>
            </div>
            <div className="d-none d-sm-block">
              <p className="mb-1 text-center fw-bold" style={{fontSize: '50px'}}>
              <CountUp end={info['number']} />
              </p>
            </div>
            <div className="small mt-2 float-end">
              <FontAwesomeIcon icon={percentageIcon} className={`${percentageColor} me-1`} />
              <span className={`${percentageColor} fw-bold`}>
                {info['percentage']}%
              </span> Since last {info['period']}
            </div>
          </Col>
        </Row>
      </Card.Body>
...
export const CircleChartWidget = ({info}) => {
      ...
      <Card style={{borderWidth: 1, borderColor: '#000'}} className="shadow-sm">
        <Card.Body>
          <Row className="d-block d-xl-flex align-items-center">
            <Col xs={12} xl={6} className="text-xl-center d-flex align-items-center justify-content-xl-center mb-3 mb-xl-0">
            ...
            <Col xs={12} xl={6} className="px-xl-0">
              ...
              {data.map((d, idx) => (
                <h6 key={`circle-element-${idx}`} className="fw-normal text-gray">
                  <FontAwesomeIcon icon={faCircle} size="1x" className={`icon icon-xs w-5 me-1`} />
                  {` ${d.category} `}{`${Math.round((d.value/total)*100)}%`}
                </h6>
              ))}
            </Col>
          </Row>
